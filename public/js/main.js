        ...
        .then(data => {
            ...
            if (data.status === 'not_found') {
                ...
                for (let suggestion of data.suggestions) {
                    ...
                    `);
                    $("#movie-list").on("click", `#movie-${index}`, function () {
                        $("#title").val(suggestion);
                        $("#submit").click(); // re-run search
                    });
                    index++;
                }
            } else if (data.status === 'found') {
                $("#choosenMovie").html("You might like these").show();
                let index = 0;
                for (let rec of data.recommendations) {
                    $("#movie-list").append(`
                        <div class="movie-li">${rec}</div>
                    `);
                    index++;
                }
            }
